...
##開始 SVM 訓練 2
##使用網格搜索（Grid search）自動找出合適的參數設定
...
GridSearchCV_clf.score(X_test, y_test)


print('Onwer\'s parameter, accuracy score: ', end = '')
print(svm.SVC(gamma = 0.001, C = 100., kernel = 'linear').fit(X_train, y_train).score(X_test,y_test))
...
#下面因每次都要關圖片才能繼續，所以先 ban 起來
'''
plt.imshow(metrics.confusion_matrix(y_test, predicted),
...
# 加入 PCA 散佈圖 
ax[0][0].scatter(X_pca[:, 0], X_pca[:, 1], c = PCA_clusters)
ax[0][0].set_title('PCA_Predicted Training Labels')
ax[1][0].scatter(X_pca[:, 0], X_pca[:, 1], c = y_train)
ax[1][0].set_title('PCA_Actual Training Labels')

# 加入 K-Means 散佈圖 
ax[0][1].scatter(X_iso[:, 0], X_iso[:, 1], c = K_Means_clusters)
ax[0][1].set_title('K-Means_Predicted Training Labels')
ax[1][1].scatter(X_iso[:, 0], X_iso[:, 1], c = y_train)
ax[1][1].set_title('K-Means_Actual Training Labels')
...
#測試讀取後的 Model
print(svc_model2.predict(X_train))
'''
